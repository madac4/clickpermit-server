...
import {
	fileExists,
	getFile,
	deleteFile as gridFsDeleteFile,
	uploadFile,
...
export const updateCompanyInfo = CatchAsyncErrors(
	async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		const { name, dba, address, city, state, zip, phone, fax, email } =
			req.body
		...
		if (!updatedCompany)
			return next(new ErrorHandler('Company information not found', 404))
...
export const getCarrierNumbers = CatchAsyncErrors(
	async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		...
		res.status(200).json(
			SuccessResponse(settings.carrierNumbers, 'Carrier numbers fetched'),
		)
	},
)
...
export const updateCarrierNumbers = CatchAsyncErrors(
	async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		...
		if (!settings) {
			...
			res.status(201).json(
				SuccessResponse(
					newSettings.carrierNumbers,
					'Carrier numbers saved',
				),
			)
			return
		}
		...
		if (!updatedSettings)
			return next(new ErrorHandler('Carrier numbers not found', 404))

		res.status(200).json(
			SuccessResponse(
				updatedSettings.carrierNumbers,
				'Carrier numbers updated',
			),
		)
	},
)
...
export const uploadCarrierFile = CatchAsyncErrors(
	async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		...
		try {
			...
			if (!updatedSettings)
				return next(new ErrorHandler('Failed to update settings', 500))

			res.status(200).json(
				SuccessResponse(null, 'File uploaded successfully'),
			)
		} catch (error: any) {
			return next(
...
export const getCarrierFiles = CatchAsyncErrors(
	async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		...
		if (!settings) {
			res.status(200).json(
				SuccessResponse({ files: [] }, 'No files found'),
			)
			return
		}
...
export const downloadFile = CatchAsyncErrors(
	async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		...
		const fileData = settings.carrierNumbers.files.find(
	...
	async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		...
		try {
			await Settings.updateOne(
				{ userId },
				{ $pull: { 'carrierNumbers.files': { filename } } },
			)
